#define _CRT_SECURE_NO_WARNINGS
#ifndef WIN32_LEAN_AND_MEAN
#define WIN32_LEAN_AND_MEAN
#endif // !WIN32_LEAN_AND_MEAN

#include<iostream>
#include<Windows.h>
#include<WinSock2.h>
#include<WS2tcpip.h>
#include<iphlpapi.h>

#include<FormatLastError.h>


using namespace std;
#pragma comment(lib,"WS2_32.lib")
#pragma comment(lib,"FormatLastError.lib")
#include<Messages.h>

#define PORT "27015"
#define BUFFER_LENGTH 1500

DWORD WINAPI ReceiveThread(LPVOID lpParam);
VOID ClientInteractionLoop(SOCKET connect_socket);

void main()
{
	setlocale(LC_ALL, "");
	cout << "CLIENT" << endl;
	//1)Init WinSOCK:
	WSADATA wsaData;
	int iResult = WSAStartup(MAKEWORD(2, 2), &wsaData);
	if (iResult!=0)
	{
		cout << "WSAStartup failed: " << iResult << endl;
		return;
	}
	//2)Задаем параметры подключения: IP-адрес сервера и порт
	struct addrinfo hints;
	struct addrinfo* result;
	ZeroMemory(&hints, sizeof(hints));
	hints.ai_family = AF_INET;
	hints.ai_socktype = SOCK_STREAM;
	hints.ai_protocol = IPPROTO_TCP;
	iResult = getaddrinfo("127.0.0.1", PORT, &hints, &result);

	if (iResult != 0)
	{
		cout << "getaddrinfo() failed: " << iResult << endl;
		WSACleanup();
		return;
	}
	//3)создаем клиентский сокет:
	CHAR szError[256] = {};
	SOCKET connect_socket = socket(result->ai_family, result->ai_socktype, result->ai_protocol);
	if (connect_socket == INVALID_SOCKET)
	{
		cout << FormatLastError(WSAGetLastError(), szError) << endl;
		cout << "Socket creation error: " << WSAGetLastError() << endl;
		freeaddrinfo(result);
		WSACleanup();
		return;
	}
	//4)Подключение к Серверу:
	iResult = connect(connect_socket, result->ai_addr, result->ai_addrlen);
	freeaddrinfo(result);
	if (iResult == SOCKET_ERROR)
	{
		DWORD dwError = WSAGetLastError();
		
		//CHAR szBuffer[256] = {};
		CHAR recvbuffer[BUFFER_LENGTH] = {};
		int bytesReceived = recv(connect_socket, recvbuffer, BUFFER_LENGTH, 0);
		if (bytesReceived > 0)
		{
			cout << "Сообщение от сервера: " << recvbuffer << endl;
		}
		cout << FormatLastError(dwError,szError) << endl;
		cout << "Unable to connect to Server." << endl;
		//cout << "Error " << dwError << ":\t" << lpBuffer << endl;

		closesocket(connect_socket);
		//freeaddrinfo(result);
		WSACleanup();
		return;
	}
	CHAR sendbuffer[BUFFER_LENGTH] = {};
	cout << "Введите ваше имя: " << endl;
	SetConsoleCP(1251);
	cin.getline(sendbuffer, BUFFER_LENGTH);
	SetConsoleCP(866);
	int iSendResult = send(connect_socket, sendbuffer, strlen(sendbuffer), 0);
	if (iSendResult == SOCKET_ERROR)
	{
		cout << "Не удалось отправить ваше имя. Ошибка: " << WSAGetLastError() << endl;
		closesocket(connect_socket);
		WSACleanup();
		return;
	}
	DWORD threadID;
	HANDLE hReceiveThread = CreateThread
	(
		NULL,
		0,
		ReceiveThread,
		(LPVOID)connect_socket,
		0,
		&threadID
	);
	if (hReceiveThread == NULL)
	{
		cout << "Failed to create receive thread. Error: " << GetLastError() << endl;
		closesocket(connect_socket);
		WSACleanup();
		return;
	}

	ClientInteractionLoop(connect_socket);
	cout << "Closing connection" << endl;
	WaitForSingleObject(hReceiveThread, INFINITE);
	CloseHandle(hReceiveThread);
	shutdown(connect_socket, SD_BOTH);
	closesocket(connect_socket);
	WSACleanup();
	
	//5)Отправка и получение данных:
	//CHAR sendbuffer[BUFFER_LENGTH] = "Hello Server";

	//do
	//{
	//	CHAR recvbuffer[BUFFER_LENGTH] = {};
	//	iResult = send(connect_socket, sendbuffer, strlen(sendbuffer), 0);
	//	if (iResult == SOCKET_ERROR)
	//	{
	//		cout << FormatLastError(WSAGetLastError(), szError) << endl;
	//		cout << "Send failed:\t" << WSAGetLastError() << endl;
	//		closesocket(connect_socket);
	//		freeaddrinfo(result);
	//		WSACleanup();
	//		return;
	//	}
	//	cout << "Bytes sent: " << iResult << endl;
	//	//do
	//	//{
	//		iResult = recv(connect_socket, recvbuffer, BUFFER_LENGTH, 0);
	//		//DWORD dwError = WSAGetLastError();
	//		//CHAR szError[256] = {};
	//		//cout << FormatLastError(dwError, szError) << endl;
	//		if (iResult > 0) cout << recvbuffer << "(" << iResult << " Bytes)" << endl;
	//		else if (result == 0)cout << "Connection closed" << endl;
	//		else cout << FormatLastError(WSAGetLastError(), szError) << endl; //cout << "Recive failed:\t" << WSAGetLastError() << endl;
	//	//} while (iResult > 0);
	//		if (strcmp(recvbuffer, DECLINE_MESSAGE) == 0)
	//		{
	//			system("PAUSE");
	//			break;
	//		}
	//		ZeroMemory(sendbuffer, BUFFER_LENGTH);
	//		SetConsoleCP(1251);
	//	cin.getline(sendbuffer, BUFFER_LENGTH);
	//	SetConsoleCP(866);
	//} while (strcmp(sendbuffer,"exit")!=0);

	//iResult = shutdown(connect_socket, SD_BOTH);
	//if (iResult == SOCKET_ERROR)
	//{
	//	cout << FormatLastError(WSAGetLastError(), szError) << endl;
	//	cout << "Shutdown falied: " << WSAGetLastError() << endl;
	//}
	//closesocket(connect_socket);
	//freeaddrinfo(result);
	//WSACleanup();
}
DWORD WINAPI ReceiveThread(LPVOID lpParam)
{
	SOCKET clientSocket = (SOCKET)lpParam;
	CHAR recvbuffer[BUFFER_LENGTH] = {};
	int iResult;
	do
	{
		ZeroMemory(recvbuffer, BUFFER_LENGTH);
		iResult = recv(clientSocket, recvbuffer, BUFFER_LENGTH, 0);
		if (iResult > 0)
		{
			cout << recvbuffer << "(" << iResult << " Bytes)" << endl;
		}
		else if (iResult == 0)
		{
			cout << "Connection closed" << endl;
			break;
		}
		else
		{
			//cout << FormatLastError(WSAGetLastError(), szError) << endl;
			break;
		
		}
	} while (iResult > 0);
	return 0;
}
void ClientInteractionLoop(SOCKET connect_socket)
{
	CHAR sendbuffer[BUFFER_LENGTH] = {};

	do 
	{
		cout << "Введите сообщение: ";
		SetConsoleCP(1251);
		std::cin.getline(sendbuffer, BUFFER_LENGTH);
		SetConsoleCP(866);

		if (strcmp(sendbuffer, "exit") == 0 || strcmp(sendbuffer, "quit") == 0)
		{
			break; 
		}

		int iResult = send(connect_socket, sendbuffer, strlen(sendbuffer), 0);
		if (iResult == SOCKET_ERROR)
		{
			//cout << FormatLastError(WSAGetLastError(), szError) << endl;
			//cout << "Shutdown falied: " << WSAGetLastError() << endl;
			std::cerr << "Send failed with error: " << WSAGetLastError() << endl;
			break;
		}

		ZeroMemory(sendbuffer, BUFFER_LENGTH); 

	} while (true);
}



